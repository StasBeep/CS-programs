...
`Laboratorywork10Variant27` - по трём заданным матрицам а(n, n), в(n, n) и с(n, n) построить матрицу x того же размера, каждый элемент которой вычисляется по формуле xi j = max {ai j , bi j , ci j} .